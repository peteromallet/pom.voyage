...
export async function getRecommendationsList(): Promise<RecommendationEntry[]> {
  return supabaseRestRequest<RecommendationEntry[]>(
    'recommendations?order=sort_order.asc&select=id,name,role_title,emoji,linkedin_url,location,body_markdown,image_url,intro_url,context,status,is_hired,is_freelancer,sort_order,created_at',
  );
}
